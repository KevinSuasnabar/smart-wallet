...
export const recordTransaction =
  (type: "income" | "expense") =>
  async (conversation: MyConversation, ctx: BotContext) => {
    ...
    if (!money.ok) { await ctx.reply("❌ Monto inválido"); return; }

    const wallets = await conversation.external(() =>
      container.listWallets({ userId }),
    );
