...
export const makeUpdateTransaction =
  ...
  async (input: UpdateTransactionInput): Promise<UpdateTransactionOutput> => {
    ...
    const existing = await deps.transactionRepo.findById(
      userIdResult.value,
      txIdResult.value,
    );
    ...
    const wallet = await deps.walletRepo.findById(
      userIdResult.value,
    ...
    <div className="flex flex-col gap-6 pb-6">
      ...
      <Card className="p-6">
        <TransactionForm
          ...
          walletId={walletId}
          onWalletChange={() => { /* disabled in edit */ }}
          onSubmit={onSubmit}
...
export const transactionKeys = {
  ...
  byWallet: (walletId: string) =>
    ['transactions', 'by-wallet', walletId] as const,
...
export const transactionsApi = {
  ...
  updateTransaction: ({
    ...
    apiPatch<TransactionResponseDTO>(
      `/wallets/${walletId}/transactions/${transactionId}`,
      body,
