...
- [x] 7.5 `git diff --name-only` (staged): `.github/workflows/deploy-frontend.yml`, `packages/web/**`, `pnpm-lock.yaml`, `eslint.config.mjs`. `packages/domain`, `packages/api`, `packages/shared-types`, `packages/infra-cdk`, `packages/infra-sls` untouched. `src/lib/api/client.ts`, `ProtectedRoute.tsx`, `ApiClientBridge.tsx`, `globals.css` untouched. See deviation notes on `pnpm-lock.yaml` and `eslint.config.mjs`.
...
6. **`eslint.config.mjs` (repo root) is modified** — `+3` lines adding `'**/*.config.{js,ts,mjs,cjs}'` to the flat-config global `ignores`. The typed ESLint project service (`projectService: true`) cannot resolve standalone root `*.ts` config files, so the new function-form `vite.config.ts` and the new `pwa-assets.config.ts` broke the `lint-staged` pre-commit hook (`eslint --fix` on staged `*.{ts,…}`). The `lint` gate (`eslint src`) is unaffected — re-run shows 0 errors, same 1 pre-existing warning. **Scope note:** this is a repo-wide config change (all `*.config.*` files, not just the two new ones, are now unlinted) shipped inside a web-PWA slice. Flagged by verify as a follow-up: needs explicit reviewer acknowledgement on the PR, or should be split into its own `chore` PR.
