...
- [ ] **T-05** Create `.github/workflows/deploy-frontend.yml` per design.md §4. Triggers: push to main only. Steps: checkout → setup pnpm + node → install → configure-aws-credentials (OIDC) → SSM read (6 params) → build web with VITE\_\* injected → s3 sync (2 passes for cache headers) → cloudfront invalidate.
