...
const isRaceLost = (e: unknown): boolean =>
  e instanceof Error && e.name === 'RecurringRaceLost';
...
export const makeMaterializeRecurrings =
  ...
  async (userIdRaw: string): Promise<MaterializeRecurringsResult> => {
    ...
    for (let iter = 0; iter < SAFETY_OUTER_LOOP; iter++) {
      const pending = await deps.recurringRepo.listPending(
        userId,
        now,
        PAGE_SIZE,
      );
      ...
      for (const recurring of pending) {
        ...
        try {
          ...
          recurring.applyMaterializationOutcome(
            outcome.nextOccurrenceAt,
            outcome.materializedAt,
          );
